...
handlers._tokens.put = function(data, callback){
    ...
    if(id && extend){
        ...
        _data.read('tokens', id, function(err, tokenData){
            if(!err && tokenData){
                // Check to the token to make sure the token isn't already expired
        ...
        callback(400, {'Error': 'Missing required field(s) or fiel(s) are invalid'})
    }
}
    ...
    if(id){
        ...
        _data.read('checks', id, function(err, checkData){
            if(!err && checkData){
                // Get the token from the headers
                var token = typeof(data.headers.token) == 'string' ? data.headers.token : false;
                // Verify that the given token is valid and belongs to the user who created the token
